...
class CustomLoginState(AuthState):
    ...
    async def on_submit(self, form_data: dict):
        """Handle login form submission with audit context."""
        self.error_message = ""
        ...
        # Use audit context manager for the entire login process
        async with with_async_audit_context(
            state=self,
            operation_type=OperationType.LOGIN,
            operation_name="user_login_attempt",
            username=username,
            login_method="form_submission",
        ):
            ip_address = self.router.session.client_ip

            # Log HTTP request (this will now be captured in audit context)
            audit_logger.info(
                "http_request",
                method="POST",
