...
class AdminState(AuthState):
    ...
    def check_auth_and_load(self):
        ...
        if not self.is_authenticated or not (
            self.is_authenticated_and_ready
            and (
                "manage_users" in self.permissions
                or "manage_suppliers" in self.permissions
            )
        ):
        ...
        try:
            with rx.session() as session:
                current_user_id = (
                    self.user_id if self.is_authenticated_and_ready else None
                )
                stmt_users = (
                ...
                self.user_stats = {
                    ...
                    "admin": sum(
                        1 for user in self.users_data if "admin" in user["role"]
                    ),
                    "supplier": sum(
