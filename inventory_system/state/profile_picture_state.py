...
class ProfilePictureState(AuthState):
    ...
    def profile_picture(self) -> str:
        ...
        if self.authenticated_user_info and self.authenticated_user_info.profile_picture:
            return self.authenticated_user_info.profile_picture
        return self._profile_picture or DEFAULT_PROFILE_PICTURE  # Default profile picture
    ...
    async def handle_profile_picture_upload(self, files: list[rx.UploadFile]):
        ...
        if not files:
            ...
            yield rx.toast.error("No file selected. Please select an image to upload.", position="bottom-right")
            return
        ...
        try:
            ...
            with file_path.open("wb") as f:
                for i in range(0, total_size, chunk_size):
                    ...
                    f.write(chunk)
                    self.upload_progress = min(round((i + len(chunk)) / total_size * 100), 100)
